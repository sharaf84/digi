...
<div class="large-3 medium-7 small-7 columns header-search-cont">
    <form action="<?= Url::to(['/store/search']) ?>" id="headerSearchForm" data-abide>
        ...
        <input type="search" name="SearchForm[key]" required pattern="alpha_numeric" placeholder="<?= Yii::t('app', 'Search') ?>" value="<?= isset($this->params['searchKey']) ? Html::encode($this->params['searchKey']) : '' ?>">
